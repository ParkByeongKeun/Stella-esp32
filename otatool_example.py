...
#    parser.add_argument('--baud', '-s', help='baud rate ', default='230400')
...
#shcho
    target.write_ota_partition(0, args.binary, args.baud)
    ...
    print('\n >>>>>>  3:Checking written firmware to ota_0 and ota_1 match factory firmware')
    ...
    print('\n >>>>>>  7:Switching to OTA slot 0 (twice in a row)')
    target.switch_ota_partition(0)
    assert_running_partition('ota_0')
    ...
    # Switch to factory app
    print('\n >>>>>>  8:Switching to factory app')
    target.erase_otadata()
    assert_running_partition('factory')

    # Switch to slot 1
    print('\n >>>>>>  9:Switching to OTA slot 1')
    target.switch_ota_partition(1)
    assert_running_partition('ota_1')
