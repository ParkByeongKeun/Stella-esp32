...
static void start_advertising(void) {
    ...
    rsp_fields.adv_itvl = BLE_GAP_ADV_ITVL_MS(150);
    ...
    /* Set advertising interval */
    adv_params.itvl_min = BLE_GAP_ADV_ITVL_MS(150);
    adv_params.itvl_max = BLE_GAP_ADV_ITVL_MS(160);
...
//  		    || (event->subscribe.attr_handle == stella_strange_chr_val_handle)) //shcho
    ...
    case BLE_GAP_EVENT_PASSKEY_ACTION:
        ...
        if (event->passkey.params.action == BLE_SM_IOACT_DISP) {
			...
			//---------------------------------------------------
			//                1000000
			//shcho force set 235711
            pkey.passkey = 235711 ;
			ESP_LOGE("passkey", "set to default 235711");
			ESP_LOGE("passkey", "set to default 235711");
			ESP_LOGE("passkey", "set to default 235711");
			//---------------------------------------------------
            ESP_LOGI(TAG, "enter passkey %" PRIu32 " on the peer side",
                     pkey.passkey);
			// -------------- display passkey ----------------------
	        size_t tx_bytes = xMessageBufferSend(passkey_msg_handle, &pkey.passkey, sizeof(uint32_t), portMAX_DELAY);
			if( tx_bytes != sizeof(uint32_t))
			{
				ESP_LOGE("xMessageBufferSend", "failed to send : passkey");
			}
			//====================================================================
