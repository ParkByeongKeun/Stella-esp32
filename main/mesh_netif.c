...
static esp_err_t mesh_netif_transmit_from_root_ap(void *h, void *buffer, size_t len)
{
    ...
    if (MAC_ADDR_EQUAL(dest_addr.addr, eth_broadcast)) {
        ...
        for (int i = 0; i < route_table_size; i++) {
            ...
            if (ESP_OK != err) {
                ESP_LOGE(TAG, "1:Send with err code %d %s", err, esp_err_to_name(err));
            }
        }
    } else {
        // Standard P2P
        ...
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "2:Send with err code %d %s", err, esp_err_to_name(err));
            return err;
        }
    }
    return ESP_OK;
...
static esp_err_t mesh_netif_transmit_from_node_sta(void *h, void *buffer, size_t len)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "3:Send with err code %d %s(%ld)", err, esp_err_to_name(err), err_count );
		err_count ++;
		if( err_count == 10 )
		{
// 1. 
        	ESP_LOGE(TAG, "3:shcho force reboot: because node is stuck when root_node reset" ) ;
			esp_restart();
// 2.
//          	ESP_LOGE(TAG, "3:shcho force esp_mesh_stop: because node is stuck when root_node reset" ) ;
//  			esp_mesh_stop(); //그냥 stop하면 Halt되는 경우가 있다.
//          	ESP_LOGE(TAG, "3:shcho force esp_mesh_start: because node is stuck when root_node reset" ) ;
//  			esp_mesh_start();
// 3.
//  	        ESP_LOGE(TAG, "3:MESH_EVENT_PARENT_DISCONNECTED와 동일하게 처리" ) ;
//  //          	mesh_layer = esp_mesh_get_layer(); //아무 효과가 없다.
//  	        esp_mesh_get_layer();
//  	        mesh_netifs_stop();
		}
    }
...
static esp_err_t mesh_driver_start_root_ap(esp_netif_t * esp_netif, void * args)
{
	...
	//shcho add 2024.0918 -------------------------------------------	
	ESP_LOGE("shcho", "set hostname(%s)", g_hostname);
	esp_netif_set_hostname(esp_netif, g_hostname);
	//----------------------------------------------------------------
...
//shcho add
uint8_t* mesh_netif_get_rootnode_mac(void)
{
