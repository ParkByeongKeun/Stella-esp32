...
void   init_db(void) 
{
    ...
    int rc =0;
    ESP_LOGI(TAG, "Initializing SPIFFS (label \"spiffs\", shared with agg_ring / history)");
   
    esp_err_t ret;
    size_t total = 0, used = 0;
    ret = esp_spiffs_info("spiffs", &total, &used);
    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "SPIFFS already mounted at /spiffs (used=%u of %u)", (unsigned)used, (unsigned)total);
    } else {
        ...
        if (ret != ESP_OK) {
            if (ret == ESP_FAIL) {
                ...
                ESP_LOGE(TAG, "Failed to find SPIFFS partition \"spiffs\"");
            } else {
    ...
    if(db_open("/spiffs/power_data.db", &db)) {
        sqlite3_close(db);
        /* SPIFFS는 stella/agg와 공유 — SQL 오류 시에도 마운트 유지 */
        return;
    }
