...
void i2c_master_init(SSD1306_t * dev, int16_t sda, int16_t scl, int16_t reset)
{
	...
	i2c_device_config_t dev_cfg = {
		...
		.device_address = I2C_ADDRESS_SSD1306,
		.scl_speed_hz = I2C_MASTER_FREQ_HZ,
	};
	i2c_master_dev_handle_t i2c_dev_handle;
	i2c_master_bus_handle_t i2c_bus_handle;

	/* Stella가 먼저 i2c_new_master_bus(I2C_NUM_1, SCL6/SDA7)로 올린 버스와 동일 핀이면 두 번째 버스를 만들지 않음(ESP-IDF CONFLICT 방지). */
	const bool pins_match_stella = (scl == 6 && sda == 7);
	if (tool_bus_handle_i2c1 != NULL && pins_match_stella) {
		ESP_LOGI(TAG, "Attach OLED to existing Stella I2C bus (tool_bus_handle_i2c1)");
		ESP_ERROR_CHECK(i2c_master_bus_add_device(tool_bus_handle_i2c1, &dev_cfg, &i2c_dev_handle));
		i2c_bus_handle = tool_bus_handle_i2c1;
	} else {
		/* Stella: GPIO6/7 OLED는 앱에서 I2C_NUM_1로 물림. Kconfig가 PORT_0이면 여기서 잘못된
		 * 컨트롤러로 ng 버스가 올라가 레거시/이중 초기화와 CONFLICT(abort)가 날 수 있음. */
		const i2c_port_t bus_port = pins_match_stella ? I2C_NUM_1 : I2C_NUM;
	...
	dev->_flip = false;
	dev->_i2c_num = (i2c_bus_handle == tool_bus_handle_i2c1 && tool_bus_handle_i2c1 != NULL)
		? I2C_NUM_1
		: (pins_match_stella ? I2C_NUM_1 : I2C_NUM);
